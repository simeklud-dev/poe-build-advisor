...
kosmetická úprava, zatím neprovedena). `GEMINI_API_KEY` je na backendu
nastavený a funkční.

**2026-07-21: migrace z Anthropic Claude na Google Gemini free tier** --
chat s AI (`advisor_chat.py`/`advisor_llm.py`) volal Claude API, což při
testování stálo reálné peníze. Přechod na Gemini free tier (zdarma API
klíč z aistudio.google.com) zachovává 1:1 stejné chování (tool-use smyčka,
kurátorovaný souhrn, capped delta, garance textové odpovědi na posledním
kole) nad jiným, bezplatným backendem -- viz `AI_BUILD_ADVISOR_PLAN.md`,
sekce "Migrace ... na Google Gemini" pro plný kontext a ověřené API tvary.
Prompt caching (Anthropic-specifická optimalizace nákladů) byl vypuštěný,
ne přeložený -- Gemini free tier nemá cenu za token, kterou by bylo co
cachovat.
...
│   │   │   ├── advisor_llm.py    # fáze 1: volitelný jednorázový komentář od Gemini
│   │   │   ├── advisor_tools.py  # fáze 2+3: nástroje pro Gemini (tool-use), curate_summary, compute_delta
...
- `POST /advisor/session/{id}/chat` -- `{message}` → `{reply, summary}`;
  Gemini v tool-use smyčce zkouší `try_item_change`/`try_node_toggle` a
...
Živé testování (2026-07-21, ještě proti Anthropic Claude backendu) odhalilo
řetězec propojených problémů a oprav v `advisor_chat.py`/`advisor_tools.py`:
...
- **Garance reálné odpovědi místo prázdného fallbacku** -- poslední kolo
  ...
  takže model musí odpovědět textem se vším, co do té doby zjistil,
  ...
  na posledním kole) neplatí -- poslední kolo má `max_output_tokens=8192`
  a vrátí text i při oříznutí.
...
by šlo snadno přehlédnout. Po migraci na Gemini (viz níže) byly obě
klíčové mechaniky (poslední kolo bez nástrojů, vyšší `max_output_tokens`
na tom kole) znovu ověřeny naživo proti reálnému PoB enginu a fungují
stejně -- Anthropic-specifický **prompt caching byl vypuštěný, ne
přeložený**: Gemini free tier nemá cenu za token, kterou by bylo co
cachovat, takže tahle optimalizace nedává na novém backendu smysl.
...
1. Backend service -- "New" → "GitHub Repo", Root Directory = kořen repa,
   ...
   volitelně `GEMINI_API_KEY` (zdarma klíč z aistudio.google.com).
